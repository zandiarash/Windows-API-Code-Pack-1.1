		...
		IInitializeWithStream, IInitializeWithItem, IInitializeWithFile
	{
		...
		private static void RegisterThumbnailHandler(string guid, ThumbnailProviderAttribute attribute)
		{
			...
			foreach (string extension in extensions)
			{
				...
				using (RegistryKey providerKey = shellExKey.CreateSubKey(Shell.Interop.HandlerNativeMethods.ThumbnailProviderGuid.ToString("B")))
				{
		...
		{
			_shellObject = ShellObjectFactory.Create((IShellItem)shellItem);
		}
