...
namespace Microsoft.WindowsAPICodePack.ShellExtensions
{
		...
		IOleWindow, IObjectWithSite, IInitializeWithStream, IInitializeWithItem, IInitializeWithFile
	{
		...
		{
			phwnd = Shell.Interop.HandlerNativeMethods.GetFocus();
		}

		HResult IPreviewHandler.TranslateAccelerator(ref MSG pmsg)
		{
		...
		void IPreviewHandlerVisuals.SetFont(ref Shell.Interop.LogFont plf)
		{
			SetFont(new Interop.LogFont(plf));
		}
		#endregion
		...
		private static void RegisterPreviewHandler(Guid previewerGuid, PreviewHandlerAttribute attribute)
		{
			...
			foreach (string extension in attribute.Extensions.Split(new char[] { ';' }, StringSplitOptions.RemoveEmptyEntries))
			{
				...
				using (RegistryKey previewKey = shellexKey.CreateSubKey(Shell.Interop.HandlerNativeMethods.PreviewHandlerGuid.ToString("B")))
				{
